...
{8.12,1.49,2.71,4.32,12.02,2.3,2.03,5.92,7.31,0.1,0.69,3.98,2.61,6.95,7.68,1.82,0.11,6.02,6.28,9.1,2.88,1.11,2.09,0.17,2.11,0.07};
	 ...
	 **/
	public static int getIC(byte[] ar)
	{
		double ic = 0;
		int keyLen = 2;
		int trials = 100;
		for(int i = 0; i < trials; i++) {
			...
			for(int k = 0; k < keyLen; k++) {
				...
				int[] freq = getFrequencies(seg);
				int tmp = 0;
				for(int letter = 0; letter < freq.length; letter++) {
					ic += (double)(Math.pow((double)freq[letter] / seg.length, 2));
				}			
			}
			ic = ic / keyLen;
			
			System.out.println("K:" + keyLen + "=>" + ic);
			
			if(ic > 0.066 && ic < 0.067) return keyLen;
			keyLen++;
		}
		return keyLen;
	}
